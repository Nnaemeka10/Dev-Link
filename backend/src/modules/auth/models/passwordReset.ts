...
export const PasswordResetModel = {
    ...
    async findBySessionToken(sessionToken: string): Promise<PasswordResetToken | null> {
        const db = getDB();
    ...
    isLocked(record: PasswordResetToken): boolean {
    ...
    async recordFailedAttempt(id: number): Promise<{ attempts: number; lockedUntil: Date | null }> {
        const db = getDB();
    ...
    async markVerifiedAndIssueSession(id: number): Promise<{ sessionToken: string; sessionExpiresAt: Date }> {
        const db = getDB();
