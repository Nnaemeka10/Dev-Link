import type { ListingRankInput } from '../types/listing.js';
...
function normalizedLogScore(value: number, normalizer: number): number {
    if (value <= 0) return 0;

    return clamp(Math.log1p(value) / Math.log1p(normalizer));
}

export function calculateTrendingScore(input: ListingRankInput): number {
