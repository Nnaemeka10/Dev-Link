...
export const ChatModel = {
    ...
    async isParticipant(conversationId, userId) {
        const db = getDB();
        const res = await db.query(`SELECT 1 FROM conversation_participants 
    ...
    async findOrCreateDirectConversation(userId, vendorId, type, contextId) {
        const db = getDB();
        ...
        try {
            ...
            if (contextId) {
                ...
                const existing = await client.query(`SELECT c.id FROM conversations c
           JOIN conversation_context ctx ON ctx.conversation_id = c.id
           WHERE ctx.${contextCol} = $1 AND c.type = $2`, [contextId, type]);
            ...
            if (!convId) {
                ...
                await client.query(`INSERT INTO conversation_participants (conversation_id, user_id, role) VALUES ($1, $2, 'guest'), ($1, $3, 'host')`, [convId, userId, vendorId]);
                // Add context
                if (contextId) {
                    const contextCol = type === 'listing' ? 'listing_id' : 'booking_id';
                    await client.query(`INSERT INTO conversation_context (conversation_id, ${contextCol}, type) VALUES ($1, $2, $3)`, [convId, contextId, type]);
                }
            }
    ...
    async createBookingSystemMessage(bookingId, userId, vendorId, listingImage, listingTitle) {
        ...
        await this.createMessage(conversationId, userId, // Sender is the system (acting as user)
        `system-${bookingId}-${Date.now()}`, // Unique idempotency key
        'system', body, listingImage ? [{ url: listingImage, mime_type: 'image/jpeg', size: 0, width: 400, height: 300 }] : []);
    },
    // Fetch messages with cursor pagination (before or after a specific sequence)
    async getMessages(conversationId, cursor, direction = 'before', limit = 50) {
        const db = getDB();
        let query = `
      SELECT m.*, 
        COALESCE(
    ...
    async getMessageById(conversationId, messageId) {
        const db = getDB();
        const res = await db.query(`SELECT m.*,
        COALESCE(
          json_agg(
            json_build_object(
              'id', a.id,
      ...
      GROUP BY m.id`, [conversationId, messageId]);
        return res.rows[0] || null;
    },
    // Insert a message with idempotency and sequence generation
    async createMessage(conversationId, senderId, clientId, type, body, attachments) {
        const db = getDB();
        const client = await db.connect();
        try {
            await client.query('BEGIN');
            ...
            const msgRes = await client.query(`INSERT INTO messages (conversation_id, sender_id, conversation_seq, client_id, type, body)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (client_id, sender_id) DO NOTHING
         RETURNING *`, [conversationId, senderId, conversationSeq, clientId, type, body]);
            ...
            if (attachments && attachments.length > 0) {
                for (const att of attachments) {
                    await client.query(`INSERT INTO message_attachments (message_id, url, mime_type, size, width, height, duration)
             VALUES ($1, $2, $3, $4, $5, $6, $7)`, [message.id, att.url, att.mime_type, att.size, att.width || null, att.height || null, att.duration || null]);
                }
            }
            // 4. Update conversation timestamps
            await client.query(`UPDATE conversations SET updated_at = NOW(), last_message_at = NOW() WHERE id = $1`, [conversationId]);
            await client.query('COMMIT');
            // Fetch full message with attachments to return
            const fullMsg = await db.query(`SELECT m.*, 
    ...
    async findOrCreateSupportConversation(userId) {
        const db = getDB();
        const client = await db.connect();
        try {
            await client.query("BEGIN");
            const existing = await client.query(`SELECT c.id
        FROM conversations c
        JOIN conversation_context ctx ON ctx.conversation_id = c.id
        JOIN conversation_participants cp ON cp.conversation_id = c.id
        ...
        LIMIT 1`, [userId]);
            ...
            const convRes = await client.query(`INSERT INTO conversations (type, last_message_at)
        VALUES ('support', NOW())
        RETURNING id`);
            const conversationId = convRes.rows[0].id;
            await client.query(`INSERT INTO conversation_context (conversation_id, type)
        VALUES ($1, 'support')`, [conversationId]);
            await client.query(`INSERT INTO conversation_participants (conversation_id, user_id, role)
        VALUES ($1, $2, 'guest')`, [conversationId, userId]);
            await client.query("COMMIT");
    ...
    async markAsRead(conversationId, userId, messageSeq) {
        const db = getDB();
        await db.query(`UPDATE conversation_participants 
       SET last_read_message_id = GREATEST(COALESCE(last_read_message_id, 0), $3)
       WHERE conversation_id = $1 AND user_id = $2`, [conversationId, userId, messageSeq]);
    },
    // Mark messages as delivered up to a specific sequence
    async markAsDelivered(conversationId, userId, messageSeq) {
        const db = getDB();
        await db.query(`UPDATE conversation_participants 
