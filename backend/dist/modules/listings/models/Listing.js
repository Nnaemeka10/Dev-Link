...
function comparisonFor(direction) {
    return direction === 'asc' ? '>' : '<';
}
function addCursorClause(clauses, values, cursor, sort, sortDirection) {
    if (!cursor)
        return;
...
export const ListingModel = {
    ...
    async findRankedByKind(kind, limit, mode) {
        const db = getDB();
        const sort = mode === 'trending' ? 'trending' : 'rating';
        const result = await db.query(`
                ${buildListingsSelect()}
    ...
    async saveListing(userId, listingId) {
        const db = getDB();
        await db.query(`INSERT INTO saved_listings (user_id, listing_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [userId, listingId]);
    },
    async unsaveListing(userId, listingId) {
        const db = getDB();
        await db.query(`DELETE FROM saved_listings WHERE user_id = $1 AND listing_id = $2`, [userId, listingId]);
    },
    // models/Listing.ts
    async findUnavailableDates(listingId) {
        const db = getDB();
        const result = await db.query(`SELECT
            ...
            ORDER BY lud.start_date ASC`, [listingId]);
        return result.rows;
    }
};
/**
 ...
 */
async function recalculateListingPrice(listingId) {
    const db = getDB();
    await db.query(`
        UPDATE listings
