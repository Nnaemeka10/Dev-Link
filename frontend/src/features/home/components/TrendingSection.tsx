...
function RankedBlock({ title, items }: { title: string; items: RankedItem[] }) {
  return (
    <div className="rounded-3xl bg-white p-6 shadow-sm md:p-8">
      <h3 className="text-[32px] font-semibold text-text-primary md:text-[38px]">{title}</h3>
        ...
        {items.map((item) => (
          <article key={item.id} className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs uppercase tracking-[0.08em] text-text-primary/48">{item.meta}</p>
              <p className="text-sm font-semibold text-accent-primary">{item.priceLine}</p>
            </div>
          </article>
        ))}
      </div>
    ...
    <section className="px-4 pb-24 pt-8 md:px-8 md:pb-20 md:pt-10">
      <div className="mx-auto max-w-7xl">
        <motion.div
          ...
          className="relative overflow-hidden rounded-[32px] bg-[radial-gradient(circle_at_top,_#123C78_0%,_#06122A_60%,_#050912_100%)] p-8 text-center text-white"
        >
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#6FA5FF]">Trending</p>
          <div className="mx-auto mt-3 inline-flex items-center gap-2 rounded-full bg-[#CF8658] px-4 py-2 text-xs font-semibold uppercase tracking-[0.08em] text-white">
            <Flame className="h-3 w-3" />
            Hottest service
          </div>
          <h2 className="mt-5 text-[40px] font-semibold leading-tight md:text-[56px]">
            {TRENDING_RESPONSE.bannerTitle}
          </h2>
          <p className="mt-3 text-base text-white/78 md:text-xl">{TRENDING_RESPONSE.bannerSubtitle}</p>
        </motion.div>
