...
//   );
// }
// components/details/SimilarVenues.tsx
import Image from "next/image";
import { Heart, Star } from "lucide-react";
  ...
  return (
    <section className={mobile ? "px-5 py-10" : "py-16"}>
      ...
      <div className={mobile ? "no-scrollbar flex gap-5 overflow-x-auto" : "grid grid-cols-3 gap-8"}>
        {venues.map((venue) => (
          <article key={venue.id} className={mobile ? "min-w-[17rem]" : ""}>
            <div className="relative h-[22rem] overflow-hidden rounded-[1.8rem] md:h-[18rem] bg-gray-100">
              {venue.primaryImage ? (
                <Image 
            ...
            <p className="mt-2 text-lg font-extrabold text-[#252423]">
              ...
              <span className="text-xs font-bold text-[#5E6588]"> {venue.priceUnit}</span>
            </p>
          </article>
        ))}
      </div>
