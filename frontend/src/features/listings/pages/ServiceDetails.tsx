...
//   );
// }
// app/listings/[id]/services/page.tsx (or equivalent ServiceDetails.tsx)
...
export default function ServiceDetails() {
  ...
  // Set default selected package on load
  if (listing && !selectedPackage && mappedPackages.length > 0) {
    setSelectedPackage(mappedPackages[0].id);
  }

  const getPackage = () => mappedPackages.find(p => p.id === selectedPackage) || mappedPackages[0];

  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-bg-primary">
        <div className="animate-pulse text-xl font-extrabold text-[#5E6588]">Loading service...</div>
      </div>
    );
  }
  ...
  if (isError || !listing) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-bg-primary">
        <div className="text-xl font-extrabold text-[#B9401D]">Failed to load service details.</div>
      </div>
    );
  }
  ...
  // Mocking provider specifics if not available directly in API 
  const provider = {
    name: listing.title, 
    avatar: listing.primaryImage?.url || "/placeholder-avatar.jpg",
  ...
  const renderAside = () => (
    <aside className="sticky top-8 rounded-[1.35rem] border border-[#E8DDD2] bg-white shadow-[0_24px_54px_rgba(34,27,18,0.1)] p-8 h-fit">
      ...
      <div className="mt-6 flex items-center justify-center gap-2 text-xs font-semibold text-[#7B7E9B]">
        <Clock className="w-4 h-4" />
        <span>Usually responds {provider.responseTime}</span>
      </div>
    </aside>
  );
